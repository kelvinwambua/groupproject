...
export default function HomePage() {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				...
				const categoriesMap = categoriesData.reduce((acc: any, cat: any) => {
					acc[cat.id] = cat;
					return acc;
				}, {});
	...
	const handleAddToCart = async (productId: string) => {
		...
		try {
			const apiUrl = `http://localhost:8000/api/users/${userId}/cart`;
			...
			if (response.ok) {
				...
				console.error(`Failed to add product ${productId} to cart:`, errorData.message || response.statusText);
